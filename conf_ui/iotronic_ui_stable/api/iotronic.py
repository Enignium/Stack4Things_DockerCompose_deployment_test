...
def plugin_inject_fleet(request, fleet_id, plugin_id, onboot):
    """Inject plugin on fleet(s)."""
    return iotronicclient(request).fleet_plugin_injection.plugin_inject_fleet(
        fleet_id, plugin_id, onboot)

def plugins_on_fleet(request, fleet_uuid):
    injections = iotronicclient(request).fleet_plugin_injection.plugins_on_fleet(
        fleet_uuid)

    result = []
    for inj in injections:
        plugin = iotronicclient(request).plugin.get(inj.plugin)
        result.append({
            "uuid": plugin.uuid,
            "name": plugin.name
        })
...
def remove_plugin_from_fleet(request, fleet_id, plugin_id):
    """Remove plugin injection from a fleet."""
    return iotronicclient(request).fleet_plugin_injection.plugin_remove_fleet(
        plugin_id, fleet_id
    )
