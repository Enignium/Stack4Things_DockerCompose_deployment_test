...
from iotronic_ui.iot.fleets_plugins import views
...
urlpatterns = [
    ...
    url(r'^(?P<plugin_id>[^/]+)/detail/$', views.PluginDetailView.as_view(),
        name='detail'),
    url(r'^(?P<plugin_id>[^/]+)/inject_fleet/$', views.InjectFleetView.as_view(),
        name='inject_fleet'),
    url(r'^(?P<plugin_id>[^/]+)/remove_fleet/$',views.RemoveFromFleetsView.as_view(),
        name='remove_fleet'),
