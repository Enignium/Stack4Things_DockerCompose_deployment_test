...
from iotronic_ui.iot.fleets_plugins import forms as project_forms
from iotronic_ui.iot.fleets_plugins import tables as project_tables
from iotronic_ui.iot.fleets_plugins import tabs as project_tabs
...
class IndexView(tables.DataTableView):
    ...
    template_name = 'iot/fleets_plugins/index.html'
...
class CreateView(forms.ModalFormView):
    template_name = 'iot/fleets_plugins/create.html'
    ...
    submit_url = reverse_lazy("horizon:iot:fleets_plugins:create")
    success_url = reverse_lazy('horizon:iot:fleets_plugins:index')
    page_title = _("Create Plugin")


class InjectView(forms.ModalFormView):
    template_name = 'iot/fleets_plugins/inject.html'
    ...
    submit_url = "horizon:iot:fleets_plugins:inject"
    success_url = reverse_lazy('horizon:iot:fleets_plugins:index')
    ...
    def get_object(self):
        try:
            ...
            redirect = reverse("horizon:iot:fleets_plugins:index")
            exceptions.handle(self.request,
...
class InjectFleetView(forms.ModalFormView):
    template_name = 'iot/fleets_plugins/inject_fleet.html'
    modal_header = _("Inject Plugin to Fleet")
    form_id = "inject_fleet_plugin_form"
    form_class = project_forms.InjectFleetPluginForm
    submit_label = _("Inject Plugin")
    submit_url = "horizon:iot:fleets_plugins:inject_fleet"
    success_url = reverse_lazy('horizon:iot:fleets_plugins:index')
    page_title = _("Inject Plugin to Fleet")

    @memoized.memoized_method
    def get_object(self):
        try:
            return iotronic.plugin_get(self.request, self.kwargs['plugin_id'], None)
        except Exception:
            redirect = reverse("horizon:iot:fleets_plugins:index")
            exceptions.handle(self.request, _('Unable to get plugin information.'), redirect=redirect)

    def get_context_data(self, **kwargs):
        context = super(InjectFleetView, self).get_context_data(**kwargs)
        args = (self.get_object().uuid,)
        context['submit_url'] = reverse(self.submit_url, args=args)
        return context
    ...
    def get_initial(self):
        ...
        # Carichiamo le fleet online
        fleets = iotronic.fleet_list(self.request)
        fleets.sort(key=lambda f: f.name)

        fleet_list = [(fleet.uuid, _(fleet.name)) for fleet in fleets]
...
class RemoveFromFleetsView(forms.ModalFormView):
    template_name = 'iot/fleets_plugins/remove_fleet.html'
    modal_header = _("Remove Plugin from Fleet(s)")
    form_id = "remove_plugin_fleet_form"
    form_class = project_forms.RemovePluginFromFleetsForm
    submit_label = _("Remove Plugin")
    submit_url = "horizon:iot:fleets_plugins:remove_fleet"
    success_url = reverse_lazy('horizon:iot:fleets_plugins:index')
    page_title = _("Remove Plugin from Fleet(s)")
    ...
    def get_context_data(self, **kwargs):
        context = super(RemoveFromFleetsView, self).get_context_data(**kwargs)
        args = (self.get_object().uuid,)
        context['submit_url'] = reverse(self.submit_url, args=args)
        return context
    ...
    def get_initial(self):
        ...
        for fleet in fleets:
            injected_plugins = iotronic.plugins_on_fleet(self.request, fleet.uuid)
            for p in injected_plugins:
                if p["uuid"] == plugin.uuid:
                    injections.append((fleet.uuid, fleet.name))
                    break

        injections.sort(key=lambda f: f[1])  # ordina per nome
        fleet_list = injections
...
class StartView(forms.ModalFormView):
    template_name = 'iot/fleets_plugins/start.html'
    ...
    submit_url = "horizon:iot:fleets_plugins:start"
    success_url = reverse_lazy('horizon:iot:fleets_plugins:index')
...
class StopView(forms.ModalFormView):
    template_name = 'iot/fleets_plugins/stop.html'
    ...
    submit_url = "horizon:iot:fleets_plugins:stop"
    success_url = reverse_lazy('horizon:iot:fleets_plugins:index')
...
class CallView(forms.ModalFormView):
    template_name = 'iot/fleets_plugins/call.html'
    ...
    submit_url = "horizon:iot:fleets_plugins:call"
    success_url = reverse_lazy('horizon:iot:fleets_plugins:index')
...
class RemoveView(forms.ModalFormView):
    template_name = 'iot/fleets_plugins/remove.html'
    ...
    submit_url = "horizon:iot:fleets_plugins:remove"
    success_url = reverse_lazy('horizon:iot:fleets_plugins:index')
...
class UpdateView(forms.ModalFormView):
    template_name = 'iot/fleets_plugins/update.html'
    ...
    submit_url = "horizon:iot:fleets_plugins:update"
    success_url = reverse_lazy('horizon:iot:fleets_plugins:index')
...
class PluginDetailView(DetailView):
    redirect_url = 'horizon:iot:fleets_plugins:index'
