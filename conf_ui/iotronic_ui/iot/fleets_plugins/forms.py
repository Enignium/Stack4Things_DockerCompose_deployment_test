...
from iotronicclient import exc as iotronic_exc
...
class InjectFleetPluginForm(forms.SelfHandlingForm):

    uuid = forms.CharField(label=_("Plugin ID"), widget=forms.HiddenInput)
    name = forms.CharField(label=_('Plugin Name'), widget=forms.TextInput(attrs={'readonly': 'readonly'}))
    onboot = forms.BooleanField(label=_("On Boot"), required=False)

    fleet_list = forms.MultipleChoiceField(
        label=_("Fleets List"),
        widget=forms.SelectMultiple(attrs={'class': 'switchable', 'data-slug': 'slug-inject-fleet'}),
        help_text=_("Select fleets")
    )

    def __init__(self, *args, **kwargs):
        super(InjectFleetPluginForm, self).__init__(*args, **kwargs)
        fleets_length = len(kwargs["initial"]["fleet_list"])
        self.fields["fleet_list"].choices = kwargs["initial"]["fleet_list"]
        self.fields["fleet_list"].max_length = fleets_length


    def handle(self, request, data):
        counter = 0
        for fleet_uuid in data["fleet_list"]:
            # ricava il nome leggibile della fleet
            fleet_name = dict(self.fields["fleet_list"].choices).get(fleet_uuid, fleet_uuid)
            try:
                result = iotronic.plugin_inject_fleet(
                    request, fleet_uuid, data["uuid"], data["onboot"]
                )
                messages.success(request, _("Plugin injected on fleet: %s") % fleet_name,
)
                counter += 1
               ...
               if "duplicate entry" in err_msg or "uniq_fleet_plugin" in err_msg:
                   ...
                   exceptions.handle(request, _("Unable to inject plugin on fleet: %s") % fleet_name)
...
class RemovePluginFromFleetsForm(forms.SelfHandlingForm):
    uuid = forms.CharField(label=_("Plugin ID"), widget=forms.HiddenInput)
    ...
    fleet_list = forms.MultipleChoiceField(
        label=_("Fleets List"),
        widget=forms.SelectMultiple(
            attrs={'class': 'switchable', 'data-slug': 'slug-remove-fleets'}),
        help_text=_("Select fleets in this pool ")
    )

    def __init__(self, *args, **kwargs):
        super(RemovePluginFromFleetsForm, self).__init__(*args, **kwargs)

        fleetslist_length = len(kwargs["initial"]["fleet_list"])
        self.fields["fleet_list"].choices = kwargs["initial"]["fleet_list"]
        self.fields["fleet_list"].max_length = fleetslist_length
    ...
    def handle(self, request, data):
        ...
        for fleet_uuid in data["fleet_list"]:
            fleet_name = dict(self.fields["fleet_list"].choices).get(fleet_uuid, fleet_uuid)
            try:
                iotronic.remove_plugin_from_fleet(request,
                                                  fleet_id=fleet_uuid,
                                                  plugin_id=data["uuid"])
                messages.success(request, _("Plugin removed from fleet: %s") % fleet_name)
                counter += 1
            except Exception:
                exceptions.handle(request, _("Unable to remove plugin from fleet: %s") % fleet_name)
