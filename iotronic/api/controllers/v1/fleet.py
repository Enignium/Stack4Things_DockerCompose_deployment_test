...
class FleetsController(rest.RestController):
	...
	'plugins': FleetPluginsController,
    }
...
from iotronic.api.controllers.v1 import plugin as plugin_controller

class FleetPluginsController(rest.RestController):

    @wsme_pecan.wsexpose(wtypes.text, types.uuid, body=wtypes.DictType(str, str))
    def post(self, fleet_uuid, body):
        """Esegue un'azione su un plugin per tutte le board della fleet."""
        plugin_name = body.get("plugin")
        action = body.get("action")

        if not plugin_name or not action:
            pecan.response.status = 400
            return "Missing 'plugin' or 'action' in body"

        try:
            from iotronic.objects import board as board_objects
            boards = board_objects.Board.list(pecan.request.context, filters={'fleet': fleet_uuid})

            results = []
            for board in boards:
                try:
                    # Esegue la chiamata al controller plugin REST esistente per board
                    result = plugin_controller.PluginController()._perform_action(board.uuid, plugin_name, action)
                    results.append({board.uuid: result})
                except Exception as e:
                    results.append({board.uuid: f"Errore: {str(e)}"})
